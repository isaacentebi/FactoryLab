    ...
    def _scoring_block(self) -> dict[str, Any]:
        ...
        return {
            ...
            "verdict_is_a_prediction": (
                ...
                "counterfactual {coin, side}, net = s * (m1 - m0) / m0 * 10^4 - 2 * f * "
                "10^4 bp, s = +1 for buy and -1 for sell, m0 the coin's mid when the return "
                "was made, m1 its mid at the horizon, f the venue's taker fee rate for the "
                "coin's market (spot for a pair, perp otherwise) in force when the return "
                "was made (venue.fee_schedule), and y = 1 when net <= 0, else 0 "
                "(declined-trade-net-v1); for a return whose answer order {coin, side} was "
                "refused (collateral check, venue rejection or terminal error) and that "
                "executed nothing else, the same net on the ordered side, and y = 1 when "
                "net > 0, else 0 (attempted-trade-net-v1); a trade whose taker fee rate "
                "the venue did not state has no y; any other return has no y. The reward is "
