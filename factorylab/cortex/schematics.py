        ...
        return {
            ...
            **institutions,
            ...
            "storage": {"working_state_max_bytes": HARD_STATE_BYTES,
                        ...
                        "and released. The policy returns a ballot shows a seat are those "
                        "delivered since its last ballot and within outcome_retention_ticks; "
                        "older ones are released unread. A vault write's venue "
                        "transaction stays bound to it, so no later write can claim it, "
                        "until it is older than every window a vault lookup can still "
                        "read (the earliest write still being looked up, less twice the "
                        "lookup's clock skew allowance). Outcome bodies are outside "
    ...
    def _scoring_block(self) -> dict[str, Any]:
        ...
        return {
            ...
            "verdict_is_a_prediction": (
                ...
                "rate on their notional, exceeds its own compute and tool cost, fixed at H "
                "or when its lots close; any counterfactual such a return named is ignored. "
                "For a return that executed nothing and named a counterfactual {coin, side}, "
                "net = s * (m1 - m0) / m0 * 10^4 - 2 * f * 10^4 - s * sum(rho) * 10^4 bp, "
                "s = +1 for buy and -1 for sell, m0 the coin's mid when the return was made, "
                "m1 its first mid timestamped at or after H, f the venue's taker fee rate "
                "for the coin's market (spot for a pair, perp otherwise) in force when the "
                "return was made (venue.fee_schedule), rho the venue's funding rate in force "
                "at each of its funding times between m0 and m1 (perps only; longs pay a "
            ...
            "abstention": (
                ...
                "credited r less p, learned as (r + prices.penalty_cap - p) / (1 + "
                "prices.penalty_cap), r the mean score before card penalty of every seat round "
            ...
            "card_penalty": (
                ...
                "router that drew the decision and the seat's own, learns (raw_score + "
                "penalty_cap - penalty) / (1 + penalty_cap), with no clip (a router's thrash "
                "charge, when it bears one, is then taken on its own map). Prices and region "
