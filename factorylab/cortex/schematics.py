    ...
    def _scoring_block(self) -> dict[str, Any]:
        ...
        return {
            ...
            "evaluator_return": (
                ...
                "verdict on a return with no world outcome first, then the latest) and, "
                "beside it, the next completed judgements up to evaluation.meta_read_share "
                "of them; the rest are passed over. A judgement's grade window closes on the "
                "tick after its cascade window released it or passed it over; one held "
                f"longer than {backstop + ev.verdict_timeout_ticks} ticks plus its window's "
                f"duration, or taken by no window within {ev.verdict_timeout_ticks} ticks, "
                "closes then. A grade that reaches a closed grade window, and a grade "
