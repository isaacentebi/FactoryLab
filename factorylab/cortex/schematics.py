    ...
    def _mechanics_block(self) -> dict[str, Any]:
        ...
        return {
            ...
            "controller": {
                ...
                "all), at the prices in force. If v > 0, with I clamped to B first "
                "(I_B = min(B, I)): I' = I_B while lambda >= B (the integrator is held at "
                "the card's own bound; S never holds it) or while kp*v + I_B >= B and "
                "v > v_previous, else I' = min(B, I_B + eta*v); otherwise I' = max(0, "
                "min(I, penalty_cap / v_previous when v_previous > 0) - decay): a price "
                "adopted above its bound unwinds on the decay schedule. D = kd*max(0, the "
