...
def reserved_return_fields(*, max_children: int | None = None,
    ...
    and a reply carrying it with a string ``reason`` is a decline whatever else it
    carries (``_validate_return``; Chapter II §II.b, the published contract is the
    enforced one).
    """
...
#: The decline, stated once as its own answer form (Chapter II §II.b). The kernel
#: reads any reply carrying ``status: "cannot"`` and a string ``reason`` as this form.
DECLINE_FORM: dict[str, Any] = {
    "type": "object",
    "description": 'a decline: a reply carrying status "cannot" and a string reason is '
                   "a decline, whatever else it carries",
...
def validate_return_sections(parsed: dict, schema: dict, validator=None, req=None,
    ...
    if (rationale_required and "rationale" not in parsed
            and parsed.get("status") != "cannot"
    ...
    if continuation and "status" in parsed and parsed["status"] != "cannot":
        ...
        # says one thing, "cannot" (``reserved_return_fields``), and a continuation is
        # not a decline, so any other value is dropped rather than voiding the turn.
