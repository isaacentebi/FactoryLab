...
def declined_reason(ret: Any) -> str | None:
    ...
    rewrites an answer whose ``status`` reads ``cannot`` (``declines``, with or
    without a ``reason``) to status ``refused`` (``ComputeMixin._invoke``), and a
    return handed in unrewritten keeps status ``ok``. A provider's own refusal names
