...
def cadence(*, sample=200, ratio=3, backstop=200):
...
def test_nearest_rank_p90_and_bounded_history_fall_with_faster_settlements():
    _, gate = cadence(sample=10)
    for i, latency in enumerate(range(10, 110, 10)):
        record(gate, latency, i)
    assert gate.slowest_period_ns(10) == 90
    for i in range(10):
        record(gate, 5, i + 10)
    assert gate.slowest_period_ns(10) == 5
    record(gate, 1_000_000, 20)
    assert gate.slowest_period_ns(10) == 5  # one outlier does not determine p90
    record(gate, 1_000_000, 21)
    assert gate.slowest_period_ns(10) == 2_000  # even observed p90 cannot exceed cap


def test_zero_latency_is_measured_without_artificial_floor():
    _, gate = cadence()
    record(gate, 0)
    assert gate.slowest_period_ns(10) == 0
    assert gate.earliest_ns(10) == 0
...
@pytest.mark.parametrize("field", ["sample", "min_ratio", "backstop"])
