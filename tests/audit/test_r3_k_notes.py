...
def test_note_utf8_bounds_and_overwrites_never_erase_on_refusal():
    ...
    assert result["text"] == "ééé" and cost == FLAT and rt.notes["ab"]["bytes"] == 8
    ...
    assert cost == FLAT and result["version"] == 2
    assert counts(rt.notes) == {"keys": 1, "bytes": 4}
...
def test_unaffordable_rent_retains_text_and_cannot_be_escaped_by_overwriting(monkeypatch):
    ...
    assert "error" not in result and cost == FLAT + 30  # the call, plus the rent it owed
...
def test_note_request_ceiling_cannot_buy_bytes_from_the_novelty_reserve(monkeypatch):
    ...
    # The ceiling is exactly what the return itself costs: the flat notebook price
    # has nothing left to draw on, and it may not reach past it into the reserve.
    ret = rt._invoke("seed-decider", replace(req, cost_ceiling=1), "producer")
...
def test_fetched_text_can_be_registered_as_a_note_in_the_continuation(monkeypatch):
    ...
    assert ret.status == "ok" and ret.cost == 1000 + FLAT
    assert ledger_items(rt, "note.put")[-1]["text"] == text
    restored = make_runtime()
    restore_runtime(restored, runtime_state(rt))
