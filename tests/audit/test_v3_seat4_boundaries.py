...
def _decision(rt, assembly, *, channel="verdict"):
    ...
    handle = rt.queue.open(
        actor=actor, event_id="seat4-input",
        propensity=PropensityRecord((assembly,), (1.0,), assembly, 0, actor, "test"),
        channel=channel, deadline_ns=10**12, parent_handle=None, cost_ceiling=1_000_000,
    )
...
def test_one_decision_round_trip_counts_its_profit_once(market, coin):
    ...
    table = table.order("opening", "round-trip", "1").fill(
        order_id="opening", coin=coin, is_buy=True, size="1", px="100",
        fee_usd="0", market=market,
    )
    table = table.order("closing", "round-trip", "1").fill(
        order_id="closing", coin=coin, is_buy=False, size="1", px="100.60",
        fee_usd="0", market=market,
    )
    payoff = table.finish("round-trip", 1_000_000).resolve(1, 200, {}).account(
        "round-trip").payoff
...
def test_evaluator_cost_card_attributes_its_measured_cost():
    ...
    card = MetricCard(
        "judge-cost", "care with scarce resources", "Judge response cost", "micro-USD",
        {"kind": "returns", "n": 1, "per": "role"}, "at most 500",
        "cost_per_return", "evaluator",
    )
    samples = CardSamples()
    samples.returned(handle="judge", assembly="eval-a", role="evaluator", window=1,
...
def test_immune_uses_the_cards_configured_sample():
    ...
    rt = make_runtime()
    card = MetricCard(
        "formation", "truthful commitments", "Valid responses", "fraction",
        {"kind": "returns", "n": 100, "per": None}, "at least 0.9",
    ...
    samples = CardSamples()
    for i in range(100):
        samples.returned(handle=f"sample-{i}", assembly="seed-decider", role="producer",
                         window=0, ret=Return(f"sample-{i}", {}, 1, "ok"))
    rt.card_samples = samples
    for i in range(1, rt.m.immune.k + 1):
        samples.returned(handle=f"bad-{i}", assembly="seed-decider", role="producer",
                         window=i, ret=Return(f"bad-{i}", {}, 1, "malformed"))
...
def test_policy_ballot_has_no_venue_write_authority(monkeypatch):
    ...
    def complete(request):
        ...
        if len(calls) == 1:
            body["tool_calls"] = [{"tool": "venue.place_market", "args": {
...
def test_registered_observation_can_enter_a_charter_proposal():
    ...
    rt.registered_observations["fresh-measure"] = {
        "description": "Public window measurement", "units": "fraction",
        "unit_range": [0, 1], "code": "def observe(facts): return 0.5",
        "version": 1, "provenance": "population", "history": [1],
    }
    rt.observation_runner = SimpleNamespace(run=lambda code, facts: (0.5, None))
    handle = _decision(rt, "seed-decider")
    card = {
        "id": "fresh-card", "norm": "useful inquiry",
        "description": "Population-defined measurement", "units": "fraction",
        "window": {"kind": "windows", "n": 1, "per": None},
        "acceptable_region": "at least 0.4", "observation": "fresh-measure",
        "answers_for": "producer",
    }
    rt._propose_amendment(handle, {
        "id": "fresh-amendment", "add": [card],
