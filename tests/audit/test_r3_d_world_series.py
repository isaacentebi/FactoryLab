...
def test_registered_multi_window_measurement_receives_bounded_world_series():
    ...
    rt.registered_observations["world-measure"] = {
        "description": "World measurement", "units": "fraction", "unit_range": [0, 1],
        "code": "def observe(facts): return 0.5", "version": 1,
    }
    seen = []
    rt.observation_runner = SimpleNamespace(run=lambda code, facts: (seen.append(facts),
                                                                    (0.5, None))[1])
    card = MetricCard("world", rt.charter.norms[2], "World", "fraction",
                      {"kind": "windows", "n": 2, "per": None}, "at least 0.1",
                      "world-measure", "all")
    samples = CardSamples()
    for index in range(2):
        rt.window = MeasureWindow(index, rt.wallet.balance)
        for tick in range(index * 600, (index + 1) * 600):
            rt._observe_delivered_event(Event(f"mid-{tick}", EventKind.MARKET_MID, tick,
                                              {"coin": "BTC", "mid": "100"}, "test"))
...
def test_world_series_survive_checkpoint_with_integer_money():
    ...
    rt = make_runtime()
    rt._manage_reserve_window()
    rt._observe_delivered_event(Event("mid", EventKind.MARKET_MID, 1,
                                      {"coin": "BTC", "mid": "123.456789"}, "test"))
    rt._observe_delivered_event(Event("funding", EventKind.FUNDING, 2,
                                      {"coin": "BTC", "rate": "-0.0001"}, "test"))
