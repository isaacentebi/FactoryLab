...
2. **Cadence.** Launch at a two-minute tick and a 60-tick consequence horizon: a trade has two hours to show its result, governance can act every six hours, compute is roughly $12 to $15 a day at seat 5's measured call rate. The horizon only bounds when a trade is judged; the population may hold positions for any length and may vote to change its own tick. The rehearsal world runs shorter. (Revised after the rehearsal, T53. The $12–$15 assumed about 30 events per tick; the rehearsal measured ≈1,687, because `LiveVenue.on_tick` broadcast one `MarketMid` per coin the venue listed and one `Funding` per listed perpetual — 212 perpetuals and 1,263 USDC-quoted spot pairs on testnet — and the shipped world never finished a tick. The broadcast is now bounded to the world's own trading markets: the manifest seed plus every registered market. `worlds/testnet.toml` seeds two perpetuals and one spot pair, so a tick delivers its `Tick` plus three `MarketMid` and two `Funding` events, six in all, and one more `MarketMid` per registered market, one more `Funding` per registered perpetual, plus the world's own fills and settled funding payments. The estimate stands on its original assumption.)
