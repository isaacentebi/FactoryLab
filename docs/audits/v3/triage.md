# Round three triage (all six seats in)
...
| T3 | Registered observations cannot be named by a card: CharterBook.validate preflights without the observation book | 2,4,5,6 | B | broken | yes, book.py:72 | fix |
| T4 | testnet.toml cannot construct: spot_pairs not on testnet (only PURR/USDC) and placeholder reserve_address refused by LiveRail; CLI hides reason as adapter_unavailable | 5,6 | B | broken | yes, testnet.toml:202,208 | fix (manifest + validate at load + name the reason) |
| T5 | Live class transfer never confirms: class_poll compares venue execution time to request nonce; treasury jams for life | 5,6 | B | broken | code read; live repro by seats 5 and 6 | fix |
| T6 | Governance activation unreachable in shipped demo (earliest 600 vs README 500); in testnet first activations at 10h/20h world time | 5,6 | B | broken | yes, cadence.py:98-101, README:35 | fix (README events >1200 or lower backstop in scripted manifests; publish schedule) + design: testnet backstop |
...
| T19 | wake publishes real venue/reserve accounts gated on env only, on any world incl. fake; mainnet check keyed on kind | 5,6 | S | broken | yes, wake.py:503-509 | fix |
...
| T27 | Ledger volume: treasury.insolvency every event; 351 MB for 500 events | 5,6 | M | unclean | seat 5 | fix (write on transition) |
...
| T30 | scripted-crash dies at wallet -$4.14 below floor 0: liquidation loss lands after affordability check | 5,6 | M | broken | seat 5 | fix or doc |
...
| T45 | about_handle undocumented in A_RETURN_MAY_INCLUDE; judges fill it with prose; 32% of paid judgements discarded (return.refused) and never fed back | 6 | S | broken | live testnet run | fix (document; fall back to router subject; feedback) |
| T46 | Every provider failure billed at the full ceiling: OpenRouterError not in _recorded_error, only UnbilledFailure releases the hold; 17% of testnet compute spent on nothing | 6 | S | broken | live testnet run | fix |
| T47 | Launch roster fails its own well_formed_rate card from window one (84% well-formed; reasoning seeds exhaust max_tokens with empty content) | 6 | S | broken | live testnet run | fix (max_tokens for reasoning seeds; reseat) + re-draft |
