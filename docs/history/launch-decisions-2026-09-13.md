# Archived launch planning, superseded

Do not use this file as current launch instructions. See `../launch-decisions.md`.
...
**Historical planning notes below are superseded; this is not launch approval.**
The September 13 rehearsal confirmed ETH fills, but also an unresolved BTC submission,
slow achieved ticks, and an activity penalty influencing orders. Current repairs require
fresh verification. Historical balances, model prices, spending estimates, and $100/3x
recommendations below are not current readings or authorized launch limits.

The experimenter's allocation is the entire Hyperliquid account, the remaining OpenRouter
credit, and Venice for additional compute. No additional allocation decision, daily inference
budget, order-size cap, gross-exposure cap, or architect-selected slippage cap is required.
The added economic limits from this session were withdrawn at the experimenter's direction.
Execution accounting, order identity/recovery, the population's charter and host operation
remain engineering concerns. Historical recommendations below do not override this decision.
