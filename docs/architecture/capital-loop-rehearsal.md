...
7. **Launch only from a checkout that contains this runbook's code, and never while
   ...
   5. resolves every authorization and plain transaction in the reserve's write-ahead
      authorization record against the finalized chain, whatever any diary now holds
      (see "The write-ahead authorization record"). It refuses while one may still
      settle or execute, or while one settled that no diary booked (a recovery: exit 3);
...
A funded run takes no supplied clock. It paces its ticks with its own live deadline
clock against the wall clock. Any other `clock_source`
refuses (`capital_loop_requires_the_live_clock`), and so does an injected `now_ns`
(`capital_loop_requires_the_wall_clock`). A harness that emits every tick at once would
otherwise run the loop faster than the bound below admitted it on.
...
before it may run ("The write-ahead authorization record").
...
- **Before any signer signs** an EIP-3009 authorization (see "One run per reserve") it
  ...
  (`F_FULLFSYNC` on macOS). A world's line also names its exact diary file (a world
  launched with `--ledger runs/foo.jsonl` names that file, and a resumed world names the
  file it resumed). If the head cannot be read or the write fails, nothing is
...
- **An append never lands on a torn line.** The writer checks the record ends in a
  ...
  which takes the lock, moves the fragment to a new `<record>.torn-<seconds>-<random>`
...
- **A damaged middle line** refuses every launch (`authorization_record_unreadable`,
  naming the line) until it is repaired:

      uv run python scripts/capital_loop_outstanding.py --repair-damaged
...
- **At every launch**, every recorded authorization not yet resolved is read twice,
  ...
  moves between two calls cannot split them. The scan runs from 300 blocks
  (`START_BLOCK_MARGIN`) before the entry's `start_block` to the first block whose
  timestamp is past its `validBefore`. While no finalized block is past it yet, the
  scan runs to the finalized block. The margin covers a head read at signing that was
  an unsafe block, which a reorg later replaced with a shorter branch. The scan filters
  by the exact nonce, so the margin cannot match another authorization. The cost is
  the same whatever the entry's age: about 600 s of blocks plus the margin, and a
  binary search for the end block. Each scan looks for `AuthorizationUsed` and for
  `AuthorizationCanceled`: an authorization cancelled from a wallet (EIP-3009
  `cancelAuthorization`) is supported. Its state reads true with a cancel log and no
  debit, so it resolves as `canceled`, dead like an expired one, with nothing to
  acknowledge. The treasury rail treats it the same way: the conversion strands
  recoverably, and nothing was paid. An entry written before `start_block` existed (legacy), or a torn one without
  ...
  - used by a world (`capital_loop`, or `treasury` from the ordinary rail), and the
    diary file it names holds the `treasury.financing` of the transfer whose confirmed
    receipt carries that nonce: resolved. An entry older than the diary field is read
    from the `ledger.jsonl` of the run directory it names. An entry with no origin was
    written before origins existed, by a capital-loop run, and is held to the same
    rule;
...
- **The cooling-off scan.** A record rolled back with its directory cannot be caught by
  ...
  with such an authorization or is a transaction the record holds (a torn line's legible
  hash included). Both pass once the window has moved past them. A log whose block the
  node cannot show is never dropped. The scan is unreadable (`cooling_off_unreadable`)
  and the launch is retried. This catches a rollback whose authorization already
...
## Every refusal and its exit

No refusal on the capital-loop path is permanent. Each has an exit, and none needs the
lock directory edited by hand, except the deliberate reset where the table says so.
Exit 3 marks a recovery (real money may have moved unbooked); every other refusal
exits 1.

| Refusal | Exit |
|---|---|
| `capital_loop_reserve_locked` | Wait for the run holding the reserve to end; the lock dies with its process. |
| `capital_loop_reserve_lock_not_held` | A caller wrote without the lock: take the lock (every script and launch does) and retry. |
| `capital_loop_lock_replaced` | Retry. Another launch replaced the file under this one; if it persists, the manual reset ("One run per reserve"). |
