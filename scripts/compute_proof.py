...
class Proof:
    ...
    def __init__(
        self, root: Path, *, dry_run: bool = False, transport: Transport | None = None,
    ...
    def clean(self, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            value = {
                k: "[REDACTED]" if str(k).lower().replace("_", "-") in {
                    "authorization", "signature", "payload", "private-key",
                    "payment-signature", "x-402-payment", "x-sign-in-with-x",
        ...
        for value in headers.values():
            ...
            try:
                ...
                signature = (envelope.get("signature")
                             or envelope.get("payload", {}).get("signature"))
                if signature:
    ...
    def step(self, name: str, action: Callable[[], dict]) -> bool:
        ...
        self.current = {"step": name, "dry_run": self.dry_run,
                        "started_at": datetime.now(UTC).isoformat(), "status": "attempting"}
        self.charge_submitted = False
        try:
        ...
        try:
            ...
            self.current.update(status="failed", error_type=type(exc).__name__,
    ...
    def status(self) -> dict:
        ...
        self.secrets.extend(os.environ.get(k, "") for k in (
            "RESERVE_PRIVATE_KEY", "VENICE_API_KEY", "OPENROUTER_API_KEY", "HL_PRIVATE_KEY",
        ))
        self.client = X402Client(transport=self.wire)
        return {"address": self.client.address, "network": BASE_NETWORK,
    ...
    def topup(self) -> dict:
        ...
        self.current.update(settlement=settlement,
    ...
    def completion(response: ModelResponse) -> dict:
        ...
        return {"model": response.model_id, "text": response.text,
                "input_tokens": response.input_tokens, "output_tokens": response.output_tokens,
                "finish_reason": response.stop_reason,
                "real_answer": isinstance(response.text, str) and bool(response.text.strip())
                and response.stop_reason == "stop", "cost_micro": response.cost_micro,
    ...
    def venice(self) -> dict:
        ...
        def transport(method, path, payload):
            response = self.wire(method, self.client.base_url + path, payload,
                                 self.client.auth_headers(path))
            if not 200 <= response.status < 300:
                raise VeniceError(response.status, "HTTP request failed")
            return response.body

        provider = VeniceProvider(transport=transport,
                                  reasoning_config={VENICE_MODEL: {"enabled": False}})
    ...
    def seller(self, model: str) -> dict:
        ...
        self.current.update(model=model, quote=asdict(quote), ceiling_micro=CEILING_MICRO,
        ...
        return {**self.completion(response), "settlement": response.raw.get("settlement"),
    ...
    def report(self) -> dict:
        ...
        verdicts = {}
        venice_proven = (
            topup.get("status") == "ok" and topup.get("settlement_reference")
            and topup.get("credited_difference_micro", 0) > 0
            and venice.get("status") == "ok" and venice.get("real_answer")
            and venice.get("cost_source") == "reported" and venice.get("cost_micro", 0) > 0
            and venice.get("balance_debit_micro", 0) > 0
            and abs(venice["balance_debit_micro"] - venice["cost_micro"]) <= 1
        )
        verdicts["venice"] = "PASS: wallet credit funded and consumed" if venice_proven else (
            "UNPROVEN: requires settled funding, credited balance and a billed real answer"
        )
        for name in ("farouter", "aispace"):
            entry = results[name]
            receipt = entry.get("settlement") or {}
            proven = (entry.get("status") == "ok" and entry.get("real_answer")
                      and receipt.get("success") is True and entry.get("settlement_reference")
                      and entry.get("cost_micro", 0) > 0)
            verdicts[name] = "PASS: settled per-request real answer" if proven else (
                "UNPROVEN: requires a successful settlement reference and paid real answer"
            )
        cash_steps = ("topup", "farouter", "aispace")
        known_spent = sum(results[n].get("cost_micro", 0) or 0 for n in cash_steps)
        uncertain = any(
            r.get("charge_submitted") and (r.get("status") != "ok" or
            (n in cash_steps and not r.get("settlement_reference")))
            for n, r in results.items()
        )
        summary = {"known_total_spent_micro": known_spent, "total_spent_uncertain": uncertain,
                   "inference_cost_micro": sum(results[n].get("cost_micro", 0) or 0
                                               for n in ("venice", "farouter", "aispace")),
                   "verdicts": verdicts, "steps": results}
    ...
    def run(self) -> int:
        ...
        ok = self.step("status", self.status)
        if self.dry_run:
        ...
        if ok:
            ok = self.step("topup", self.topup)
        if ok:
            ok = self.step("venice", self.venice)
        if ok:
            for name, model in (("farouter", FAROUTER_MODEL), ("aispace", AISPACE_MODEL)):
                if not self.step(name, lambda model=model: self.seller(model)):
                    ok = False
                    break
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        return Proof(Path.cwd(), dry_run=args.dry_run).run()
    except ProofRefused as exc:
        print(str(exc), file=sys.stderr)
        return 1
    except Exception as exc:
        print(f"Compute proof failed ({type(exc).__name__}); retain all attempt markers",
              file=sys.stderr)
        return 1
